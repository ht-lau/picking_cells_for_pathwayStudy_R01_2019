...
filter.essential.genes <- function(essential.df,        # essential genes df
                                   retain.percentile    # a length-of-two vector with 
...
filtered.reactomes <- apply(essential, 1, filter.essential.genes, retain.percentile = c(0.45, 0.55))
names(filtered.reactomes) <- essential$Pathway

filtered.cells <- ccle.pathways
filtered.cells[1:11] <- filtered.reactomes
