...
add.quantile <- function(test) {
  ...
  for (i in 1:nrow(test)) {
    if (test[i, 1] <= quantile.in.pathway[[1]]) {
      test[i, 2] <- 0
    } else if (test[i, 1] <= quantile.in.pathway[[2]]) {
      test[i, 2] <- 25
...
test <- 
for (i in 1:11) {
  colnames(sum.gene.expression[[i]])[2] <- str_c("Quantile.", pathway.names[[i]])
}
...
for (i in 1:21) {
  ...
  cells.list[[i]] <- select(cells.list[[i]], Cells = 4, 2, 3)
}
...
# split the cells.list data into 2 dfs. One contains the selected cells, the other contains the quantile data
cells.list.quantile <- cells.list %>% select(Cells, contains("Quantile"))
cells.list <- cells.list %>% select(-contains("Quantile"))
