...
cell.pathway.quantile <- reduce(sum.gene.expression, 
...
cell.pathway.quantile <- select(cell.pathway.quantile, 3, contains("Quantile"))
...
cells.list <- left_join(cells.list, cell.pathway.quantile, by = c("Cells", "Pathways"))

# fill the NAs in the picked cells column with the quantile number, then remove the information, turn back to wide format
cells.list <- cells.list %>%
  mutate(Picked = ifelse(is.na(cells.list$Picked), 
                         cells.list$Quantiles, 
                         cells.list$Picked)) %>%
  select(-Quantiles) %>%
